...
export function OrderStatusBadge({ status }: { status: string }) {
  return (
    <Badge
      variant="outline"
      className={`no-default-hover-elevate no-default-active-elevate border-0 font-medium ${STATUS_STYLES[status] || "bg-muted text-muted-foreground"}`}
...
export function PaymentStatusBadge({ status }: { status: string }) {
  return (
    <Badge
      variant="outline"
      className={`no-default-hover-elevate no-default-active-elevate border-0 font-medium ${PAYMENT_STYLES[status] || "bg-muted text-muted-foreground"}`}
