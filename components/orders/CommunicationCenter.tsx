...
function matchesFilter(msg: CommMessage, tab: FilterTab): boolean {
  ...
  if (tab === "system")      return msg.sender_type === "system";
  return true;
}
...
function MessageBubble({ msg }: { msg: CommMessage }) {
  ...
  const showExpand  = msg.body.length > 200;
  const displayBody = !expanded && showExpand ? msg.body.slice(0, 200) + "…" : msg.body;
    ...
    <div className={`flex flex-col gap-1 ${isOutbound ? "items-end" : "items-start"}`}>
      ...
      <div className={`flex items-center gap-1.5 text-[10px] text-muted-foreground ${isOutbound ? "flex-row-reverse" : ""}`}>
        {isOutbound
          ...
          : <ArrowDownLeft className="h-3 w-3 text-muted-foreground/60" />}
        {/* sender badge */}
        <span className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] font-medium ${senderCfg.cls}`}>
          {senderCfg.icon}
          {senderCfg.label}
        </span>
        <span className="opacity-50">→</span>
        {/* recipient badge */}
        <span className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] font-medium ${recipCfg.cls}`}>
          {recipCfg.icon}
          {recipCfg.label}
        </span>
        {/* channel */}
        ...
        {/* time */}
        <span className="opacity-50">{formatRelative(msg.created_at)}</span>
      </div>

      {/* Body */}
      <div
        className={`max-w-[85%] rounded-lg px-3 py-2 text-xs leading-relaxed whitespace-pre-wrap border ${
          isOutbound
      ...
      >
        {msg.subject && (
          <p className="font-semibold mb-1 text-[11px]">Subject: {msg.subject}</p>
        )}
        {displayBody}
        {showExpand && (
          <button
            onClick={() => setExpanded((e) => !e)}
            className="block mt-1 text-primary hover:underline text-[10px]"
          >
      ...
      {/* Extracted data (for inbound seller replies) */}
...
export function CommunicationCenter({ order, onRefresh }: CommunicationCenterProps) {
    ...
    <Card data-testid="card-communication-center">
      ...
      <CardContent className="space-y-3">
          ...
          <div className="rounded-md border bg-muted/20 p-3 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <Select
                ...
                onValueChange={(v) => setComposeChannel(v as "email" | "sms" | "both")}
              >
        ...
        <div
          ...
          data-testid="feed-communications"
        >
          {loading && (
          ...
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-8 text-center space-y-1">
              ...
              <p className="text-xs text-muted-foreground/70">
                {filter === "all"
                  ? "Outbound seller contacts and inbound replies will appear here."
